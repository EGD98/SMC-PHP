...
if($_POST) {
   // error_reporting(0);
    ...
    if($conexion->connect_errno){
        ...
        $querys= "SELECT * FROM horarios WHERE  (id_horario != '$id_update') AND (espacio = '$nombre_espacio') AND ((('$fecha_i' BETWEEN fecha_inicio AND fecha_final) OR ('$fecha_f'  BETWEEN fecha_inicio AND fecha_final )) OR (( fecha_inicio BETWEEN '$fecha_i' AND '$fecha_f') OR (fecha_final  BETWEEN '$fecha_i' AND '$fecha_f' ))) AND ((('$hora_i' BETWEEN hora_inicio AND hora_final) OR ('$hora_f'  BETWEEN hora_inicio AND hora_final )) OR (( hora_inicio BETWEEN '$hora_i' AND '$hora_f') OR (hora_final  BETWEEN '$hora_i' AND '$hora_f' ))) ";
        //echo ($querys);
